...
const account1 = {
  ...
  movementsDates: [
    ...
    "2024-09-15T14:11:59.604Z",
    "2024-09-18T17:01:17.194Z",
    "2024-09-19T23:36:17.929Z",
    "2024-09-20T10:51:36.790Z",
  ],
  currency: "RUB",
  locale: "pt-PT",
};
...
function displayMovements(acc, sort = false) {
  ...
  movs.forEach(function (value, i) {
    ...
    <div class="movements__row">
          ...
          <div class="movements__value">${Intl.NumberFormat(acc.locale, options).format(value)}</div>
        </div>
    `;
...
function calcPrintBalance(acc) {
  ...
  labelBalance.textContent = `${Intl.NumberFormat(acc.locale, options).format(acc.balance)}`
}
...
function calcDisplaySum(acc) {
  ...
  labelSumIn.textContent = `${Intl.NumberFormat(acc.locale, options).format(incomes)}`;

  const out = acc.movements
    .filter((mov) => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${Intl.NumberFormat(acc.locale, options).format(Math.abs(out))}`;
  
  labelSumInterest.textContent = `${Intl.NumberFormat(acc.locale, options).format(incomes + out)}`;
}
